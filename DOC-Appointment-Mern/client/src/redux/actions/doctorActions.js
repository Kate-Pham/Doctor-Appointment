...
export const getAllDoctors = createAsyncThunk(
    ...
    async (_, thunkApi) => {
        try {
            const res = await API.get("http://localhost:8080/api/v1/doctor/get-all");
            return res.data;
        } catch (error) {
...
export const getDoctorDetails = createAsyncThunk(
    ...
    async (id, thunkApi) => {
        try {
            const res = await API.get(`http://localhost:8080/api/v1/doctor/get-details/${id}`);
...
export const addDoctor = createAsyncThunk(
    ...
    async (formData, thunkApi) => {
        try {
            const res = await API.post("http://localhost:8080/api/v1/doctor/add", formData, {
                headers: {
...
export const updateDoctor = createAsyncThunk(
    ...
    async ({ id, formData }, thunkApi) => {
        try {
            const res = await API.patch(`http://localhost:8080/api/v1/doctor/update/${id}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
...
export const deleteDoctor = createAsyncThunk(
    ...
    async (id, thunkApi) => {
        try {
            const res = await API.delete(`http://localhost:8080/api/v1/doctor/delete/${id}`);
            console.log("API response:", res);
            return res.data;
        } catch (error) {
